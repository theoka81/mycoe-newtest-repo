...
public abstract class HibernateGenericCsdDao<T, ID extends Serializable> implements GenericDao<T, ID> {
	protected Logger logger = LogManager.getLogger("SCMCORE");

	protected static int JDBC_BATCH_SIZE = 2;
	private Class<T> persistentClass;
	
	private SessionFactory csdSessionFactory;
	

	@SuppressWarnings("unchecked")
	public HibernateGenericCsdDao() {
		this.persistentClass = (Class<T>) ((ParameterizedType) getClass().getGenericSuperclass())
				.getActualTypeArguments()[0];
	}


	public SessionFactory getCsdSessionFactory() {
		return csdSessionFactory;
	}

	@Autowired
	public void setCsdSessionFactory(SessionFactory csdSessionFactory) {
		this.csdSessionFactory = csdSessionFactory;
	}
	
	protected Session getCsdSession() {
		return csdSessionFactory.getCurrentSession();
	}
	...
	@Override
	public T get(ID id) {
		T instance = (T) getCsdSession().get(getPersistentClass(), id);
		return instance;
	}
	...
	public T getWithNoWaitLock(ID id) {
		...
		T instance = (T) getCsdSession().get(getPersistentClass(), id, lockOptions);
		return instance;
	}
	...
	public T load(ID id, boolean lock) {
		T entity;
		if (lock)
			entity = (T) getCsdSession().load(getPersistentClass(), id, LockOptions.UPGRADE);
		else
			entity = (T) getCsdSession().load(getPersistentClass(), id);
	...
	public void lock(T item) {
		getCsdSession().buildLockRequest(LockOptions.UPGRADE).lock(item);
	}
	...
	public T merge(T entity) {
		return (T) getCsdSession().merge(entity);
	...
	public T save(T entity) {
		getCsdSession().saveOrUpdate(entity);
		return entity;
	}
	...
	@Override
	public void commit() {
		getCsdSession().getTransaction().commit();
	}
	...
	public void setFlushModeToCommit() {
		getCsdSession().setFlushMode(FlushMode.COMMIT);
	}

	@Override
	public void setFlushModeToAuto() {
		getCsdSession().setFlushMode(FlushMode.AUTO);
	}

	@Override
	public void setFlushModeToManual() {
		getCsdSession().setFlushMode(FlushMode.MANUAL);
	}

	@SuppressWarnings("hiding")
	public <T> void refresh(T entity) {
		getCsdSession().refresh(entity);
	}
